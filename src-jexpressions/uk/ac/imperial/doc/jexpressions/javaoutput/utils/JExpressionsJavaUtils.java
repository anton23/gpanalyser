...
public class JExpressionsJavaUtils {
	...
	public static double normalMinProduct2Acc(double mA, double mB, double thetaSq, double mA2, double mB2, double add, 
				double mAacc, double mBacc, double addAcc) {
		double theta = Math.sqrt(Math.max(thetaSq, 0.0));
		double safePhi = safe_Phi(mB2 - mA2, theta*add);
		return 
		mAacc * safePhi + mBacc * (1.0 - safePhi) - theta * addAcc * safe_phi(mB2 - mA2, theta*add);					
	}
	
	public static double normalMinProductAcc(double mA, double mB, double thetaSq, double mA2, double mB2, double add, double mAacc, double mBacc, double addAcc) {
		double theta = Math.sqrt(Math.max(thetaSq, 0.0));
		double safePhi = safe_Phi(mB - mA, theta);
		return 
		mAacc * safePhi + mBacc * (1.0 - safePhi) - theta * addAcc * safe_phi(mB - mA, theta);	
	}
