	...
	public static void drawChart(String windowTitle,String command,double[][] data,double minx,double dx,double miny,double dy,String xlabel, String ylabel, String zlabel){
		...
		for (int ix = 0; ix<data.length-1; ix++){
			...
			for (int iy = 0; iy<data[0].length-1; iy++){
				...
				if (Double.isInfinite(z1)||Double.isInfinite(z2)||Double.isInfinite(z3)||Double.isInfinite(z4)) continue; 
		...
		surface.setColorMapper(new ColorMapper(new ColorMapRainbow(), zmin, zmax, new org.jzy3d.colors.Color(1,1,1,1f)));
