...
public class SpatialClosureVisitorUniversal extends MomentCountTransformerWithParameters implements ICombinedProductExpressionVisitor, IExpressionVariableVisitor
{
	...
	public SpatialClosureVisitorUniversal(CombinedPopulationProduct _moment, int _maxOrder, Map<State, Map<State, Integer>> _distance)
	{
	...
	public void visit(CombinedProductExpression _e)
	{
        ...
        // Check all pairs are lie within distance
        ...
        for (State s1 : states) {
          for (State s2 : states) {
            ...
            if (m_distance.get(s1).containsKey(s2)) {
              withinDist = true;
            }
          }
        }
			  if (withinDist) {
			    ...
			    // Mean field closure
			    //PCTMCLogging.info(Arrays.toString(terms.toArray()));
		      result = ProductExpression.create(terms);
			  }
			}
			// Mean field closure
			...
			{
			  PCTMCLogging.error("Function not implemented yet");
			  System.exit(0);
	...
	private void genIsserlisClosure(int _order)
	{
		...
		if (_order % 2 == 0)
		{
			Set<Set<List<CombinedPopulationProduct>>> allPartitionsIntoPairs = SpatialClosureVisitorUniversal.<CombinedPopulationProduct> getAllPartitionsIntoPairs(Arrays.asList(states));
